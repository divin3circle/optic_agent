...
export const idlFactory: IDL.InterfaceFactory = ({ IDL }) => {
  ...
  const Icrc21ConsentMessageMetadata = IDL.Record({ utc_offset_minutes: IDL.Opt(IDL.Nat), language: IDL.Text });
  const Icrc21ErrorInfo = IDL.Record({ description: IDL.Text });
  const LimitOrderKey = IDL.Record({ timestamp: IDL.Nat, tickLimit: IDL.Nat });
  const LimitOrderValue = IDL.Record({ userPositionId: IDL.Nat, token0InAmount: IDL.Nat, owner: IDL.Principal, token1InAmount: IDL.Nat });
  const Result = IDL.Variant({ ok: IDL.Nat, err: Error });
  const Result_1 = IDL.Variant({ ok: IDL.Text, err: Error });
  const Result_16 = IDL.Variant({ ok: IDL.Vec(IDL.Tuple([IDL.Nat, IDL.Vec(IDL.Nat8)])), err: Error });
  const Result_2 = IDL.Variant({ ok: IDL.Bool, err: Error });
  const Result_30 = IDL.Variant({ ok: IDL.Record({ amount0: IDL.Nat, amount1: IDL.Nat }), err: Error });
  const SwapArgs = IDL.Record({ amountIn: IDL.Text, zeroForOne: IDL.Bool, amountOutMinimum: IDL.Text });
  const SwapInfo = IDL.Record({ err: IDL.Opt(Error__1), status: IDL.Variant({ Failed: IDL.Null, CreditCompleted: IDL.Null, Created: IDL.Null, Completed: IDL.Null }), tokenIn: Token, tokenOut: Token, amountOutFee: IDL.Nat, amountIn: IDL.Nat, amountOut: IDL.Nat, amountInFee: IDL.Nat });
  const Time = IDL.Nat;
  const Token__1 = IDL.Record({ address: IDL.Principal, standard: IDL.Text });
  const Transfer = IDL.Record({ to: Account, fee: IDL.Nat, token: IDL.Principal, from: Account, memo: IDL.Opt(IDL.Vec(IDL.Nat8)), index: IDL.Nat, amount: IDL.Nat, standard: IDL.Text });
  const UserPositionInfoWithId = IDL.Record({ id: IDL.Nat, tickUpper: IDL.Nat, tokensOwed0: IDL.Nat, tokensOwed1: IDL.Nat, feeGrowthInside1LastX128: IDL.Nat, liquidity: IDL.Nat, feeGrowthInside0LastX128: IDL.Nat, tickLower: IDL.Nat });
  const WithdrawArgs = IDL.Record({ fee: IDL.Nat, token: IDL.Text, amount: IDL.Nat });
  const WithdrawToSubaccountArgs = IDL.Record({ fee: IDL.Nat, token: IDL.Text, subaccount: IDL.Vec(IDL.Nat8), amount: IDL.Nat });
  
  const _SERVICE = IDL.Service({
    activeJobs: IDL.Func([], [], []),
    addLimitOrder: IDL.Func([IDL.Record({ positionId: IDL.Nat, tickLimit: IDL.Nat })], [Result_2], []),
    allTokenBalance: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Record({ token0: IDL.Nat, token1: IDL.Nat }), err: Error })], ['query']),
    approvePosition: IDL.Func([IDL.Principal, IDL.Nat], [Result_2], ['query']),
    batchRefreshIncome: IDL.Func([IDL.Vec(IDL.Nat)], [IDL.Variant({ ok: IDL.Variant({ ok: IDL.Nat, err: Error }), err: Error })], ['query']),
    checkOwnerOfUserPosition: IDL.Func([IDL.Principal, IDL.Nat], [Result_2], ['query']),
    claim: IDL.Func([IDL.Record({ positionId: IDL.Nat })], [Result_30], []),
    decreaseLiquidity: IDL.Func([IDL.Record({ liquidity: IDL.Text, positionId: IDL.Nat })], [Result_30], []),
    deleteFailedTransaction: IDL.Func([IDL.Nat, IDL.Bool], [Result_2], []),
    deposit: IDL.Func([IDL.Record({ amount: IDL.Nat, fee: IDL.Nat, token: IDL.Text })], [Result], []),
    depositAllAndMint: IDL.Func([IDL.Record({ amount0: IDL.Nat, amount0Desired: IDL.Text, amount1: IDL.Nat, amount1Desired: IDL.Text, fee0: IDL.Nat, fee1: IDL.Nat, positionOwner: IDL.Principal, tickLower: IDL.Nat, tickUpper: IDL.Nat })], [Result], []),
    depositAndSwap: IDL.Func([IDL.Record({ amountIn: IDL.Text, amountOutMinimum: IDL.Text, tokenInFee: IDL.Nat, tokenOutFee: IDL.Nat, zeroForOne: IDL.Bool })], [Result], []),
    depositFrom: IDL.Func([IDL.Record({ amount: IDL.Nat, fee: IDL.Nat, token: IDL.Text })], [Result], []),
    depositFromAndSwap: IDL.Func([IDL.Record({ amountIn: IDL.Text, amountOutMinimum: IDL.Text, tokenInFee: IDL.Nat, tokenOutFee: IDL.Nat, zeroForOne: IDL.Bool })], [Result], []),
    getAdmins: IDL.Func([], [IDL.Variant({ ok: IDL.Vec(IDL.Principal), err: Error })], ['query']),
    getAvailabilityState: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ whiteList: IDL.Vec(IDL.Principal), available: IDL.Bool }), err: Error })], ['query']),
    getCachedTokenFee: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ token0Fee: IDL.Nat, token1Fee: IDL.Nat }), err: Error })], ['query']),
    getClaimLog: IDL.Func([], [IDL.Variant({ ok: IDL.Vec(IDL.Text), err: Error })], ['query']),
    getCycleInfo: IDL.Func([], [IDL.Variant({ ok: IDL.Variant({ ok: IDL.Nat, err: Error }), err: Error })], []),
    getFailedTransactions: IDL.Func([], [Result_16], ['query']),
    getFeeGrowthGlobal: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ token0: IDL.Nat, token1: IDL.Nat }), err: Error })], ['query']),
    getInitArgs: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ token0: IDL.Principal, token1: IDL.Principal, token0Fee: IDL.Nat, token1Fee: IDL.Nat, tickSpacing: IDL.Nat, maxLiquidityPerTick: IDL.Nat, maxTick: IDL.Nat, minTick: IDL.Nat, maxLiquidity: IDL.Nat, tickMath: IDL.Record({ getTick: IDL.Func([IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getSqrtPriceX64: IDL.Func([IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getAmount0ForLiquidity: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getAmount1ForLiquidity: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getLiquidityForAmounts: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getAmount0ForLiquidityX128: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getAmount1ForLiquidityX128: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []), getLiquidityForAmountsX128: IDL.Func([IDL.Nat, IDL.Nat], [IDL.Variant({ ok: IDL.Nat })], []) }) }), err: IDL.Null })], ['query']),
    getJobs: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ jobs: IDL.Vec(IDL.Record({ interval: IDL.Nat, lastRun: Time, name: IDL.Text, timerId: IDL.Opt(IDL.Nat) })), level: IDL.Variant({ Active: IDL.Null, Inactive: IDL.Null }) }), err: Error })], ['query']),
    getLimitOrderAvailabilityState: IDL.Func([], [IDL.Variant({ ok: IDL.Bool, err: Error })], ['query']),
    getLimitOrderStack: IDL.Func([], [IDL.Variant({ ok: IDL.Variant({ ok: IDL.Nat, err: Error }), err: Error })], ['query']),
    getLimitOrders: IDL.Func([], [IDL.Variant({ ok: IDL.Vec(LimitOrderValue), err: Error })], ['query']),
    getMistransferBalance: IDL.Func([Token], [Result], []),
    metadata: IDL.Func([], [IDL.Variant({ ok: IDL.Record({ fee: IDL.Nat, key: IDL.Text, sqrtPriceX96: IDL.Nat, tick: IDL.Nat, liquidity: IDL.Nat, token0: Token, token1: Token, maxLiquidityPerTick: IDL.Nat, nextPositionId: IDL.Nat }), err: Error })], ['query']),
